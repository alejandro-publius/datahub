...
- #18690 **(Ingestion / SQL Profiling)** MySQL and Postgres profiling now runs under `AUTOCOMMIT` instead of one implicit transaction per table, so profiling no longer pins InnoDB read views (MySQL) or holds Postgres idle-in-transaction (blocking `VACUUM`). Profile **values can now differ** for concurrently-written tables — `min`, `max`, `COUNT(*)`, `uniqueCount`, quantiles, histograms, and sample values each come from a different snapshot, so a single profile may be internally inconsistent (e.g. `uniqueCount` > `rowCount`). To restore the prior per-table transactional behavior, set `profiling.isolation_level: TRANSACTIONAL` on the source, or set `DATAHUB_PROFILING_FORCE_TRANSACTIONAL=true` on the ingestion executor for a fleet-wide kill switch that wins over every recipe. An invalid level name fails the run loudly; a level the server rejects (e.g. a proxy that disallows `AUTOCOMMIT`) silently reverts to the prior behavior with a warning, so a rejecting proxy needs no recipe change to keep profiling. Reverting restores the pre-change profile output with no migration; the kill switch changes subsequent runs only, so re-ingest after an incident.
