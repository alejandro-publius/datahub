...
- #18690 **(Ingestion / SQL profiling)** The SQLAlchemy profiler now runs MySQL and Postgres profiling under `AUTOCOMMIT` instead of holding one implicit transaction open across every profiling query for a table. This prevents the long-lived transaction that pinned InnoDB read views (MySQL) / held Postgres idle-in-transaction and blocked VACUUM. Profile **values can now differ** for concurrently-written tables: `min`, `max`, `COUNT(*)`, `uniqueCount`, quantiles, histograms, and sample values each come from a different snapshot, so a single profile may be internally inconsistent (e.g. `uniqueCount` > `rowCount`). With `query_combiner_enabled` on (the default) the profiler issues a small number of combined statements — roughly one per profiling stage (row count; cardinality; numeric stats) — so metrics within a stage share a snapshot, but metrics in different stages do not; the `uniqueCount` > `rowCount` case above is exactly such a cross-stage disagreement. Histograms, quantiles, distinct-value frequencies and sample values are never combined and are each issued separately. With the combiner disabled, no bound applies. This is an accepted trade-off — analytical profiling tolerates minor inconsistency, and it is safer than the long-transaction alternative. If the database or a proxy rejects the `AUTOCOMMIT` session setting, the profiler records a warning and falls back to one transaction per table, while still emitting profiles. Set `profiling.profile_use_autocommit: false` (or `DATAHUB_PROFILE_USE_AUTOCOMMIT=false`) to restore the previous per-table transactional profiling if you need a consistent snapshot more than you need the VACUUM/undo-log behaviour. Other SQL platforms are unchanged.
...
- `DATAHUB_PROFILE_USE_AUTOCOMMIT` (default `true`) — Fleet-wide default for whether MySQL and Postgres profiling runs under `AUTOCOMMIT`. Set `false` to restore one transaction per table (consistent snapshot, but reinstates the long-lived transaction that blocks VACUUM on Postgres / pins an InnoDB read view on MySQL). An explicit `profiling.profile_use_autocommit` value in the recipe wins where both are present. Superseded by `profiling_consistency` in a future release.
