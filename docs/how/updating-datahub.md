...
- #18690 **(Ingestion / SQL profiling)** The SQLAlchemy profiler now runs MySQL and Postgres profiling under `AUTOCOMMIT` instead of holding one implicit transaction open across every profiling query for a table. This prevents the long-lived transaction that pinned InnoDB read views (MySQL) / held Postgres idle-in-transaction and blocked VACUUM. **Postgres** is effectively a no-op: under the default READ COMMITTED each statement already took a fresh snapshot, so AUTOCOMMIT changes only the transaction state, not the visibility — there is no consistency regression there. **MySQL** is the behavior change: InnoDB runs REPEATABLE READ by default, so under the old behavior every profiling query for a table shared one snapshot, while under AUTOCOMMIT each statement takes its own snapshot. Profile **values can now differ** for concurrently-written MySQL tables: `min`, `max`, `COUNT(*)`, `uniqueCount`, quantiles, histograms, and sample values each come from a different snapshot, so a single profile may be internally inconsistent (e.g. `uniqueCount` > `rowCount`). This is an accepted trade-off — analytical profiling tolerates minor inconsistency, and it is safer than the long-transaction alternative. Other SQL platforms are unchanged.
